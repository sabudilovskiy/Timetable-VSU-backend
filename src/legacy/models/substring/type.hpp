...
class SubString
{
   public:
    ...
    SubString(Args&&... args) noexcept(
        std::is_nothrow_constructible_v<std::string, Args&&...>)
        : data_(std::forward<Args>(args)...)
    {
    }

    template <typename Arg>
    SubString& operator=(Arg&& args) noexcept(
        std::is_nothrow_assignable_v<std::string, Arg>)
    {
