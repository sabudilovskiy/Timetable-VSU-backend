...
namespace detail
{
template <utils::ConstexprString str>
...
template <typename T>
requires std::is_integral_v<T> constexpr T ParseIntegral(std::string_view sv)
{
...
constexpr std::string_view GetPureWord(std::string_view token)
{
    ...
    if (end != std::string_view::npos && token[end] == ' ')
        end--;
...
#define DECLARE_OPENAPI_ENUM(NAME, TYPE, ...)                               \
    ...
    {                                                                       \
        static constexpr auto tokens =                                      \
            ::openapi::detail::split_tokens<#__VA_ARGS__>();                \
        static constexpr auto names = ::openapi::detail::get_names(tokens); \
        static constexpr auto values =                                      \
            ::openapi::detail::get_values<NAME>(tokens);                    \
    }
