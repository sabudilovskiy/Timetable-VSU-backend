          ...
          userver::formats::serialize::To<ResponseInfo>)
{
    ...
    auto matcher_header = [&]<typename H, auto HTraits>(
    ...
    auto matcher_cookie = [&]<typename C, auto CTraits>(
        const CookieProperty<C, CTraits>&)
    {
        STATIC_ASSERT_FALSE(
            "Openapi doesnt support cookie in responses. Sorry :(");
    };
    auto matcher_json_body = [&]<typename B, auto BTraits>(
