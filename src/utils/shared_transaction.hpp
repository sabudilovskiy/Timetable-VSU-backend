...
совместных транзакций не является threadsafe, но является safe относительно
совместного использования. Стандартное применение предполагает получение
интрузивного птр в каждом методе контроллера, заполнение его при помощи
FillSharedTransaction, чтобы гарантировать наличие какое-либо транзакции. По
умолчанию выполняется коммит в деструкторе, чтобы избежать этого поведения нужно
задать противоположное. Благодаря этому явный коммит не нужен.
*/
