 ...
 protected:
  ...
  static void SerializeField(HttpResponse&, const EmptyBody&, const Field&) {
    auto bad = []<bool flag = false>() {
      static_assert(flag, "Found property from body, but body marked empty");
    };
  }
  ...
  static void SerializeField(HttpResponse& response,
    ...
    //на самом деле там триллион параметров, для куки нужен отдельный все-таки
    //проперти и отдельная обработка
  ...
  static auto GetBody() {
    if constexpr (type_of_body == TypeOfBody::Json) {
      //было бы неплохо поддерживать не только возврат как объекта, но пока так
  ...
  static void SerializeTuple(HttpResponse& response, Body& body, Tuple&& tuple,
