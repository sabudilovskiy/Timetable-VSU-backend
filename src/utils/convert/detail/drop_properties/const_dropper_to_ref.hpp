...
struct ConstDropperToRef final
{
    ...
    static auto ResultType()
    {
        return type_holder<decltype(
            ConstDropperToRef::Do(std::declval<const T&>()))>{};
    }
    ...
    static TupleType GetTuple(const ValueType& v)
    {
    ...
    static decltype(auto) HandleMember(const Member& member)
    {
        return member();
    }
    ...
    static decltype(auto) HandleMember(const Member& member)
    {
        ...
        if constexpr (IsConvertAll<NestedType>)
        {
            ...
            using ResultTypeHelper =
                decltype(ConstDropperToRef<NestedType>::ResultType());
            using ResultType = typename ResultTypeHelper::value_type;
            std::optional<ResultType> result = std::nullopt;
            if (member().has_value())
            {
                result = ConstDropperToRef<NestedType>::Do(member().value());
            }
            return result;
        }
        else
    ...
    static decltype(auto) HandleMember(const Member& member)
    {
        ...
        if constexpr (IsConvertAll<NestedType>)
        {
            ...
            for (auto& elem : value)
            {
                result.emplace_back(ConstDropperToRef<NestedType>::Do(elem));
            }
            return result;
        }
        else
    ...
    static decltype(auto) HandleMember(const Member& member)
    {
        ...
        if constexpr (IsConvertAll<NestedType>)
        {
            ...
            if (value.has_value())
            {
                ...
                {
                    result.emplace_back(
                        ConstDropperToRef<NestedType>::Do(elem));
                }
            }
            return result;
        }
