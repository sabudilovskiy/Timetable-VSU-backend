...
UserType Parse(const userver::formats::json::Value& value, userver::formats::parse::To<UserType>){
    if (!value.IsString()){
        throw std::runtime_error(fmt::format("Expected string type, but got: {}", utils::GetType(value)));
    }
    auto raw_value = value.As<std::string>();
    return Parse(std::string_view{raw_value}, userver::formats::parse::To<UserType>{});
}
}  // namespace timetable_vsu_backend::models